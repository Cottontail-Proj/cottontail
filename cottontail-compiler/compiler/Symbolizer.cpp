...
void Symbolizer::visitSelectInst(SelectInst &I) {
  ...
  IRBuilder<> IRB(&I);
  char*name = getInstructionDebugInfo(I);
  //printf("SelectInst name = %s \n", name);
  //unsigned opcode = I.getOpcode(); // Get the opcode of the instruction
  //printf("************** opcode of the select instruction: %d\n", opcode);
  const char* opcode_name = I.getOpcodeName(); // Get the opcode of the instruction
  //printf("************** opcode of the select instruction: %s\n", opcode_name);
  Value *varName = IRB.CreateGlobalStringPtr(name);
  Value *opName = IRB.CreateGlobalStringPtr(opcode_name);
  Value *branchType = ConstantInt::get(IRB.getInt8Ty(), 0);
  auto runtimeCall = buildRuntimeCall(IRB, runtime.pushPathConstraint,
                                      {{I.getCondition(), true},
                                       {I.getCondition(), false},
                                       {getTargetPreferredInt(&I), false}, {varName, false}, {opName, false}, {branchType, false}});
...
void Symbolizer::visitReturnInst(ReturnInst &I) {
  // Upon return, we just store the expression for the return value.
    ...
    // Treat the pointer as a unique block identifier
  //printf("Basic Block Address (Block ID:return): %p \n", BB);
...
int analyzeBranch(llvm::BranchInst &I) {
    // Check if the branch instruction is conditional
    if (!I.isConditional()) {
        return 0; // Not a conditional branch
    }

    llvm::BasicBlock *Parent = I.getParent();
    llvm::Function *F = Parent->getParent();
    if (!F) {
        llvm::errs() << "Branch instruction is not part of a function.\n";
        return -1;
    }

    // Initialize PostDominatorTree for the function
    llvm::PostDominatorTree PDT;
    PDT.recalculate(*F);

    // Get the true and false successor blocks
    llvm::BasicBlock *TrueBlock = I.getSuccessor(0);
    llvm::BasicBlock *FalseBlock = I.getSuccessor(1);

    // Check if TrueBlock and FalseBlock converge in a common post-dominator
    llvm::BasicBlock *CommonPostDom = PDT.findNearestCommonDominator(TrueBlock, FalseBlock);

    // If TrueBlock and FalseBlock converge at a post-dominator, it's an if-else
    if (CommonPostDom && CommonPostDom != TrueBlock && CommonPostDom != FalseBlock) {
        return 2; // if-else branch
    }

    return 1; // single if branch
}
...
void Symbolizer::visitBranchInst(BranchInst &I) {
  ...
  Value *varName = IRB.CreateGlobalStringPtr(name);
  Value *opName = IRB.CreateGlobalStringPtr(opcode_name);
  Value *branchType = ConstantInt::get(IRB.getInt8Ty(), analyzeBranch(I)); // 1 - if ; 2 - if-else
  auto runtimeCall = buildRuntimeCall(IRB, runtime.pushPathConstraint,
                                      {{I.getCondition(), true},
                                       {I.getCondition(), false},
                                       {getTargetPreferredInt(&I), false}, {varName, false}, {opName, false}, {branchType, false}});
...
void Symbolizer::visitStoreInst(StoreInst &I) {
  ...
  //printf("THX: we call Symbolizer::visitStoreInst??? name = %s \n", I.getPointerOperand()->getName().str().c_str());
  auto V = I.getValueOperand();
  char * name = getInstructionDebugInfo(I);
  std::string store_name = std::string(name) + "_" + I.getPointerOperand()->getName().str();
  Value *varName = IRB.CreateGlobalStringPtr(store_name);
  ...
  for (auto type_it = gep_type_begin(I), type_end = gep_type_end(I);
    ...
    if (auto *structType = type_it.getStructTypeOrNull()) {
      ...
      unsigned elementSize =
        ...
        //printf ("THX: THIS IS A STRUCT TYPE! elementSize = %d;\n", elementSize);
...
std::string getCaseLocationString(const llvm::SwitchInst::CaseHandle &caseHandle) {
    // Get the label for the case
    llvm::BasicBlock *caseBlock = caseHandle.getCaseSuccessor();

    // Find the debug info for this case label
    llvm::MDNode *location = caseBlock->getTerminator()->getMetadata("dbg");
    
    if (location) {
        llvm::DILocation *diLoc = llvm::dyn_cast<llvm::DILocation>(location);
        if (diLoc) {
            // Get the line and column
            unsigned line = diLoc->getLine();
            unsigned column = diLoc->getColumn();
            
            // Get the file name from DIFile
            std::string filename = "";
            llvm::DIFile *diFile = diLoc->getScope()->getFile();
            if (diFile) {
                filename = diFile->getFilename().str();
            }

            // Get the function name from DISubprogram
            std::string functionName = "";
            llvm::DISubprogram *diSubprogram = diLoc->getScope()->getSubprogram();
            if (diSubprogram) {
                functionName = diSubprogram->getName().str();
            }

            // Concatenate filename, function name, line, and column
            return filename + "_" + functionName + "_" + std::to_string(line) + "_" + std::to_string(column);
        }
    }
    
    // Return an empty string if no debug information is available
    return "";
}
...
void Symbolizer::visitSwitchInst(SwitchInst &I) {
  ...
  auto *Switch = dyn_cast<SwitchInst>(&I);
   ...
   //llvm::Value *condition = Switch->getCondition();
   // TODO: find another way to get the switch name, e.g., grep the source code and map the relations using line number
   std::string switch_name;
  if (llvm::LoadInst *loadInst = llvm::dyn_cast<llvm::LoadInst>(condition)) {
    llvm::Value *loadedFrom = loadInst->getPointerOperand();
    if (loadedFrom->hasName()) {
        switch_name = loadedFrom->getName().str();
    } else {
      llvm::errs() << "Unnamed pointer variable.\n";
    }
  }
  ...
  for (auto &Case : Switch->cases()) {
    ...
    std::string loc_case = key + "_" + std::to_string(CaseValue->getValue().getSExtValue());
    value.insert({CaseValue->getValue().getSExtValue(), loc_case});
  }
  
  switch_case_map.insert(std::pair<std::string, std::map<int32_t, std::string>>{key, value});
  assert(switch_case_map.size() != 0 && "switch_case_map should not be zero\n");
  ...
  for (auto &caseHandle : I.cases()) {
    ...
    IRB.CreateCall(runtime.pushPathConstraint,
                   {caseConstraint, caseTaken, getTargetPreferredInt(&I), varName, opName, branchType});
  }
}
...
void Symbolizer::tryAlternative(IRBuilder<> &IRB, Value *V) {
  ...
  //char*name = getInstructionDebugInfo( llvm::dyn_cast<llvm::Instruction*>(V));
  //const char* name = "on_tryAlternative";
  Value *varName = IRB.CreateGlobalStringPtr(V->getName());
  //printf("tryAlternative name = %s \n", V->getName().str().c_str());
  Value *opName = IRB.CreateGlobalStringPtr("try");
  Value *branchType = ConstantInt::get(IRB.getInt8Ty(), 0);
  if (destExpr != nullptr) {
    ...
    auto *pushAssertion = IRB.CreateCall(
        ...
        {destAssertion, IRB.getInt1(true), getTargetPreferredInt(V), varName, opName, branchType});
...
char* Symbolizer::getInstructionDebugInfo(Instruction &I) {
    if (DILocation *Loc = I.getDebugLoc()) {
        // Retrieve line and column numbers
        unsigned line = Loc->getLine();
        unsigned col = Loc->getColumn();

        // Get file name without the directory path
        StringRef fullFileName = Loc->getFilename();
        StringRef fileName = llvm::sys::path::filename(fullFileName);

        // Get the function name
        Function *func = I.getFunction();
        StringRef funcName = func->getName();

        // Get the instruction type (opcode name)
        const char* instrType = I.getOpcodeName();

        // Combine instruction type, file name, function name, line, and column into a key string
        //std::string key = (std::string(instrType) + "_" + fileName.str() + "_" + funcName.str() + "_" + std::to_string(line) + "_" + std::to_string(col));
        std::string key = fileName.str() + "_" + (funcName.str() + "_" + std::to_string(line) + "_" + std::to_string(col)) + "_" + std::string(instrType);
        //std::string key = fileName.str() + "_" + (funcName.str() + "_" + std::to_string(line) + "_" + std::to_string(col));

        /*
        // Append only the first operand's name if available
        if (I.getNumOperands() > 0) {
            key += ":";
            Value *operand = I.getOperand(0);  // Get the first operand
            if (operand->hasName()) {
                key += operand->getName().str();
            } else {
                key += "null";  // Handle unnamed operands
            }
        }
        */

        //printf("Name in getInstructionDebugInfo = %s\n", key.c_str());

        // Return the C-style string (char*)
        return strdup(key.c_str());  // Don't forget to free this later
    } else {
        // Handle the case when no debug information is available
        return strdup("no_debug_info");
    }
}
