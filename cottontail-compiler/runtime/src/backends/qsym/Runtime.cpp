...
GenericTree<std::string> tree("root");
bool is_saved = 0;
...
SymExpr registerExpression(const qsym::ExprRef &expr) {
  ...
  if (allocatedExpressions.count(rawExpr) == 0) {
    ...
    //std::cout << "+++ THX allocate expression: " << rawExpr->toZ3Expr() << "\n";
...
public:
  ...
  void pushInputByte(size_t offset, uint8_t value) {
    ...
    //std::cout << "@@@@@THX in pushInputByte offset = " << offset << " value = " << value << "\n";
...
void _sym_initialize(void) {
  ...
  if (std::holds_alternative<NoInput>(g_config.input)) {
    std::cerr
        << "Performing fully concrete execution (i.e., without symbolic input) -- (cottontail debug version)"
        << std::endl;
    return;
  }
...
void _sym_push_path_constraint(SymExpr constraint, int taken,
                               uintptr_t site_id, char* name, char* op_name, int branchType) {
  if (constraint == nullptr)
    return;
  
  if (name == nullptr) assert("Get a name without name!!!");

  g_solver->addJcc(allocatedExpressions.at(constraint), taken != 0, site_id, name, op_name, branchType);
}

SymExpr _sym_get_input_byte(size_t offset, uint8_t value) {
  g_enhanced_solver->pushInputByte(offset, value);

  SymExpr sym_expr = registerExpression(g_expr_builder->createRead(offset));
...
void _sym_notify_ret(uintptr_t site_id) {
  ...
  if (g_call_stack_manager.call_stack_.size() == 0){
