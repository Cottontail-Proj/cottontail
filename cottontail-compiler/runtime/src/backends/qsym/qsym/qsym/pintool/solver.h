...
public:
  ...
  bool checkOnly(const std::string& postfix=""); // THX
  void addJcc(ExprRef, bool, ADDRINT, char*, char*, int);
...
protected:
  ...
  void negatePath(ExprRef, bool, char* name, char* op_name, bool unvisited);
  void solveOne(z3::expr);

  void checkFeasible();

  //z3::expr evaluateSingleExpression(z3::expr org_expr, z3::expr sym_expr, z3::expr con_expr);
  //bool evaluateSymbolicExpression(z3::expr expr, const std::vector<uint8_t> &input);

  void negatePathESCT(ExprRef, bool, char* name, char* op_name, bool unvisited);\
  //double getEnvD(const char* varName);
