...
void tryAlternative(V value, SymExpr valueExpr, F caller) {
  if (valueExpr) {
    _sym_push_path_constraint(
        ...
        true, reinterpret_cast<uintptr_t>(caller), "", "", -1);
  }
}
  ...
  for (size_t i = 0; i < length; i++) {
    ...
    _sym_push_path_constraint(
        _sym_build_not_equal(
            ...
            cExpr),
        /*taken*/ 1, reinterpret_cast<uintptr_t>(SYM(strchr)), "", "", -1);
    ++shadowIt;
  }
...
int SYM(memcmp)(const void *a, const void *b, size_t n) {
  ...
  _sym_push_path_constraint(allEqual, result == 0,
                            reinterpret_cast<uintptr_t>(SYM(memcmp)), "", "", -1);
  return result;
}
...
int SYM(bcmp)(const void *a, const void *b, size_t n) {
  ...
  _sym_push_path_constraint(allEqual, result == 0,
                            reinterpret_cast<uintptr_t>(SYM(bcmp)), "", "", -1);
  return result;
}
